...
impl Starknet {
    ...
    pub(crate) fn generate_new_block_and_state(&mut self) -> Felt {
        ...
        if !self.config.lite_mode {
            ...
            let commitments = match Handle::try_current() {
                Ok(handle) => task::block_in_place(|| handle.block_on(commitments_future)),
    ...
    pub(crate) fn restart_pre_confirmed_block(&mut self) {
...
mod tests {
    ...
    pub(crate) fn setup_starknet_with_no_signature_check_account_and_state_capacity(
        ...
        let acc = Account::new(
            ...
            dummy_key_pair(),
            account_class.try_generate_hash().unwrap(),
            "Custom",
            account_class.into(),
    ...
    fn assert_expected_predeclared_account_classes() {
        ...
        for class_hash in [
            ...
            let contract = starknet
                ...
                .unwrap();
            assert_eq!(contract.try_generate_hash().unwrap(), class_hash);
        }
    }
