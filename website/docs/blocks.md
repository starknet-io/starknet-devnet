...
Eligible system-lane transactions are always selected FIFO before a user ordering policy is consulted. The `fifo` policy then selects the oldest nonce-eligible user transaction. The `starknet` policy considers only nonce-eligible user account heads whose maximum L2 gas price meets the active proposal's L2 gas price, ordering them by descending tip and then descending transaction hash; underpriced transactions remain received. The `random` policy chooses deterministically among nonce-eligible user account heads. A transaction with a nonce gap stays received until earlier transactions for its account execute.
