...
The `fifo` policy selects the oldest nonce-eligible transaction. The `starknet` policy prioritizes nonce-eligible account heads using Starknet-like fee ordering. The `random` policy chooses deterministically among nonce-eligible account heads. A transaction with a nonce gap stays received until earlier transactions for its account execute.
